...
docs_index = faiss.read_index(f'quora_faiss_index_v3.index')
doc_ids = joblib.load(f"all_quora_doc_ids_v3.joblib")
docs = joblib.load(f"all_quora_doc_texts.joblib")
doc_embeddings = joblib.load(f"all_quora_doc_embeddings_v3.joblib") 
...
def search_faiss(dataset, query, top_k=5):
    ...
    D, I = docs_index.search(query_embedding_np, top_k)
...
def search(query: str = Form(...), algorithm: str = Form(...)):
    results = search_faiss("quora",query, top_k=10)
